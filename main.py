...
from pm25 import get_data_from_mysql, write_data_to_mysql, get_avg_pm25_from_mysql,get_pm25_by_county
...
def get_county_pm25(county):
    ...
    if len(result) == 0:
        return Response(
            json.dumps(
                {"result":"取得資料失敗","message":f"查無{county}資料"},
                ensure_ascii=False,
        )
    )
...
def index():
    time = now_time()
    return render_template("index.html", x=time, name="Lennox")
